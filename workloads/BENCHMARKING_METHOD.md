...
- CoreMark + Dhrystone: `workloads/` via `python3 workloads/run_benchmarks.py`

Third-party suites (downloaded to `workloads/third_party/`):
...
Sources and pinned revisions are tracked in `workloads/third_party/SOURCES.md`.
...
2. Add a minimal hosted layer only when required:
   - syscall ABI + `printf` is already available via `avs/runtime/freestanding/`
