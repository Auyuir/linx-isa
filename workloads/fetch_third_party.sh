...
OUT_DIR="$ROOT/workloads/third_party"
...
fetch_github_tarball() {
  ...
  if is_populated_dir "$dest"; then
    echo "== skip ${owner}/${repo}@${ref} (already present: $dest)"
    {
      ...
      echo "  - extracted_to: workloads/third_party/$(basename "$dest")"
    } >>"$SOURCES_MD"
    return 0
  fi
...
This folder is populated by `workloads/fetch_third_party.sh`.

This repo already vendors CoreMark and Dhrystone under `workloads/`.
