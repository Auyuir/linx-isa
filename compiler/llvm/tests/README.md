...
```bash
CLANG=/path/to/clang ./compiler/llvm/tests/run.sh
```

Run (linx32):

```bash
CLANG=/path/to/clang TARGET=linx32-linx-none-elf OUT_DIR=./compiler/llvm/tests/out-linx32 ./compiler/llvm/tests/run.sh
```
...
```bash
python3 ./compiler/llvm/tests/report_isa_coverage.py
```
