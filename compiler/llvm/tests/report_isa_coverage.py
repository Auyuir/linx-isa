...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Directory containing per-test output folders (default: compiler/llvm/tests/out).",
    )
    ap.add_argument(
        "--spec",
        type=Path,
    ...
    ap.add_argument(
        "--llvm-project",
        type=Path,
        default=Path(__file__).resolve().parents[4] / "llvm-project",
    ...
    if not args.out_dir.exists():
        ...
        print(
            "hint: run ./compiler/llvm/tests/run.sh first (or set --out-dir)",
            file=sys.stderr,
        )
        return 2
