...
### Testing
- [ ] Run test suite: `compiler/llvm/tests/run.sh`
...
- [x] `compiler/llvm/tests/analyze_coverage.py` - Coverage analysis
- [x] `compiler/llvm/templates/` - Implementation templates
...
python3 tools/isa/gen_llvm_tablegen.py \
  ...
  --out compiler/llvm/LinxInstrInfo.td

# Run tests
cd compiler/llvm/tests
CLANG=~/llvm-project/build-linxisa-clang/bin/clang ./run.sh

# Analyze coverage
python3 compiler/llvm/tests/analyze_coverage.py --verbose
```
...
- Use templates in `compiler/llvm/templates/` as starting points
