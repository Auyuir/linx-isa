...
implement tests under `tests/qemu/` (runtime) and `compiler/llvm/tests/`
...
- Requirement:
  - `llvm-objdump -d` MUST be able to decode and print a mnemonic token for
    every mnemonic enumerated in `isa/spec/current/linxisa-v0.1.json` when fed
    the generated decode vectors (`compiler/llvm/tests/gen_disasm_vectors.py`).
- Pass criteria:
  - `python3 compiler/llvm/tests/analyze_coverage.py --fail-under 100` exits 0.
