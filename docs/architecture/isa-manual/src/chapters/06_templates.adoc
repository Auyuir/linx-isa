...
Restartability contract (v0.1 bring-up profile):
...
* `ESAVE` / `ERET`-class exception/restore template blocks (Tile/TEPL domain)
...
The minimal libc startup in `toolchain/libc/src/crt0.s` uses `FENTRY` + `FRET.STK` around a direct call:
...
golden sources (`isa/golden/v0.1/`) and the canonical catalog (`isa/spec/current/linxisa-v0.1.json`).
