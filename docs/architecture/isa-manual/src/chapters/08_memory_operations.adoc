...
* Address footprint is 2D and derived from descriptor state:
  - dimensions from `LB0/LB1` (`B.DIM`)
  - layout format from `B.ARG` (both compact `format` and named v0.3 forms such as `ND2ZN.normal`, `DN2NZ.normal`,
    `NZ2DN.canon`, `NORM.normal`)
...
Canonical strict-v0.3 disassembly form for tile memory blocks is:

* `BSTART.TMA TLOAD, <DataType>` for tile loads
* `BSTART.TMA TSTORE, <DataType>` for tile stores
* Compatibility PAR-style forms are accepted for input only and normalized to typed forms.
