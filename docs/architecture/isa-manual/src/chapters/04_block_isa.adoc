...
The v0.1 bring-up profile defines **three block forms**. Toolchains and implementations MUST enforce the legality
...
In the v0.1 draft, BARG includes (non-exhaustive):
...
**BARG lifetime rules (v0.1 draft):**
...
The v0.1 draft constrains certain combinations (examples):
...
Block Split forms; this manual treats their detailed execution semantics as outside scope of the v0.1 bring-up unless
explicitly specified in the golden sources (`isa/golden/v0.1/`).
...
Bring-up contract (v0.1 profile):
...
The v0.1 catalog includes the following descriptor families:
...
NOTE: The v0.1 draft notes that `C.BSTOP` is encoded as all-zeros in the 16-bit space; it functions both as an
