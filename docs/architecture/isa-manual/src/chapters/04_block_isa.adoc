...
* **Block attributes and hints**
  - `B.ATTR`: sets generic block attributes/hints; strict TMA execution does not source dtype/layout/pad from `B.ATTR`.
...
* `PTO TLOAD`:
  `BSTART.TMA(TLOAD)` + `B.DIM(LB0/LB1/LB2)` + `B.ARG` + `B.IOR*` + `B.IOT/B.IOTI`
* `PTO TSTORE`:
  `BSTART.TMA(TSTORE)` + `B.DIM(LB0/LB1/LB2)` + `B.ARG` + `B.IOR*` + `B.IOT/B.IOTI`
* `PTO TMATMUL`:
...
* `TLOAD/TSTORE` dtype comes from `BSTART.TMA`, and pad policy comes from `B.ARG` (not `B.ATTR`).
