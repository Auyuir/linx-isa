...
in the golden sources (`isa/golden/v0.3/`).
...
Vector block bodies may mix scalar-uniform and vector instructions; scalar-uniform forms use scalar block-local queues
(`t/u`) while vector forms use vector/tile-local namespaces.
...
* Header descriptors (`B.ARG`, `B.ATTR`, `B.IOR`, `B.IOT`, `B.DIM`, `B.TEXT`) pack command fields.
* `BSTOP` finalizes and dispatches the command to the selected PE.
* The PE executes and returns live-out/status metadata.
* `BROB` tracks command completion and retires blocks in-order.
