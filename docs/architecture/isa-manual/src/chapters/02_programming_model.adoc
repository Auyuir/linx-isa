...
Address translation (v0.1 bring-up profile):
...
NOTE: The v0.1 `reg5` table also includes special spellings used by the assembler/disassembler for queue operations
...
Bring-up conventions (v0.1 draft):
...
named **ACR0..ACR15**, which are organized as a tree rooted at **ACR0** (see <<system-acr-tree>>). In the v0.1 draft:
