[[system]]
== System and privileged architecture (v0.1 draft)

This chapter defines the v0.1 draft privileged architecture: Access Control Rings (ACR), privileged system registers
...
Typical software role mapping (informative, v0.1 draft):
...
The v0.1 draft uses the following notation for comparable rings:
...
* prepare/commit of a **system-call block** (v0.1 draft mechanism).

The v0.1 draft distinguishes:
...
`ACR_ENTER` state restoration (v0.1 draft):

1. `CSTATE` is restored from `ECSTATE_ACRn` (all architecturally defined fields).
2. `BPC` is restored from `EBPC_ACRn`, and execution is scheduled at that block start.
3. If `RRA_Type=RRAT_RESTORE`, `BSTATE` is restored from `EBSTATE` (profile-defined storage).
4. If `RRA_Type=RRAT_RESTORE`, the block argument state (`BARG`; see <<progmodel-barg>>) is restored from
   `EBARG_ACRn` (profile-defined for non-scalar block types).
5. If `RRA_Type=RRAT_RESTORE`, `TPC` is restored from `ETPC_ACRn` (the trap source instruction).
...
`SERVICE_REQUEST` is the v0.1 draft trap entry mechanism used to transfer control from a managed ring to a managing
ring.
...
1. If the event is a floating-point exception, set the corresponding sticky flags in `CSTATE.flags`.
...
3. Save the trap source block address (`BARG.BPC`) into `EBPC_ACRm`.
4. Save block argument state into `EBARG_ACRm` and record the trap source block type in `EBARG_ACRm.BlockType`.
  * Save the trap source instruction `TPC` into `ETPC_ACRm`.
  * If the trap source block is `MPAR`, `MSEQ`, `VPAR`, or `VSEQ`, record the interrupted `GroupID` in
    `EBARG_ACRm.GroupID`.
...
9. Populate trap reporting registers:
  * For a SYNC_SERVICE_REQUEST: set `TRAPNO_ACRm.E = 1`, then write `TRAPNO_ACRm.TRAPNUM`, `TRAPNO_ACRm.CAUSE`, and
    `TRAPARG0_ACRm`.
  * For an ASYNC_SERVICE_REQUEST: set `TRAPNO_ACRm.E = 0`, then write `TRAPNO_ACRm`/`TRAPARG0_ACRm` according to the
    interrupt source definition (Xproxy-defined).

NOTE: The v0.1 draft specifies `BPC` vectoring explicitly. At the start of the trap handler block, `TPC` is expected
to equal `BPC` (see <<progmodel-pc>>).

[[system-trap-routing]]
===== Routing rules (v0.1 draft)

`SERVICE_REQUEST` routing is profile-defined for interrupts. The v0.1 draft defines the following routing rules for
...
`ACRC` requests a `SERVICE_REQUEST` and immediately commits its block. `ACRC` MUST be the last instruction in its
block.
...
Valid `request_type` encodings (v0.1 draft):
...
On `ACRC`, hardware saves the current block address (`BPC`) and the instruction address (`TPC`) into the managing
ring's trap-save registers (`EBPC_ACRm` and `ETPC_ACRm`).

Software may choose how execution resumes when it later uses `ACRE` to return:

* **Redo** the `ACRC`: leave `EBPC_ACRm` and `ETPC_ACRm` unmodified.
* **Advance** to the next instruction: update both `EBPC_ACRm` and `ETPC_ACRm` to `ETPC_ACRm + 4` (the `ACRC`
  instruction length), which typically points at the next block start marker.

[[system-exception-routing]]
====== Default exception routing

The v0.1 draft defines the following default managing ring selection for synchronous exceptions:
...
For `E_SCALL` (software calls), routing is defined by the `ACRC` `request_type` rules above.

[[system-bstate-ebstate]]
=== Block engine state (`BSTATE` / `EBSTATE`)

A block execution unit may maintain internal state beyond the first-layer architectural state. The v0.1 draft names
this internal per-block state **BSTATE**.
...
Trap save/restore for scalar block types (`STD`, `SYS`, `FP`) is implemented using privileged SSRs (see
<<ssr-acr-scoped>>):

* save `BARG.BPC` → `EBPC_ACRm`
* save `BARG.BPCN` → `EBPCN_ACRm`
* save remaining `BARG` fields → `EBARG_ACRm`
* save `TPC` → `ETPC_ACRm`

For complex block types (`VPAR`, `VSEQ`, `MPAR`, `MSEQ`), the v0.1 draft additionally records `GroupID` in
`EBARG_ACRm.GroupID`. The bulk storage and restoration of other block-engine state is profile-defined (e.g. via tile
registers and/or memory).

Dirty vs clean `BSTATE` (v0.1 draft):
...
[[system-bstate-min]]
==== Minimum required `BSTATE` / `EBSTATE` contents (v0.1 bring-up profile)

Implementations targeting the v0.1 bring-up profile MUST implement at least the following `BSTATE` fields and MUST
...
* `EBPC_ACRn` / `EBPCN_ACRn` / `EBARG_ACRn` / `ETPC_ACRn` save trap source state for later recovery.

These SSRs use IDs of the form `0xnfxx` and are visible in the global SSR ID space. In the v0.1 privilege model they
MUST be accessible from ACR0 and ACR1; other ACRs may ignore accesses or may trap with `E_ISSR`.

[[system-mm]]
=== Memory management (TTBR0/TTBR1) and IOMMU (v0.1 bring-up profile)

This section defines the v0.1 bring-up MMU and IOMMU contract. It uses an **ARM-inspired TTBR0/TTBR1 split** for the
...
* `[63:48]` — reserved, SHOULD be zero (ASID ignored in v0.1 for `TTBR1`).
* `[47:12] BasePA` — physical base address of the level-0 translation table (4 KiB aligned).
* `[11:0]` — reserved, MUST be zero.

[[system-mm-tcr]]
==== `TCR_ACR1` layout (v0.1 subset)

Only the following `TCR_ACR1` fields are defined in the v0.1 bring-up profile. All other bits are reserved and MUST
...
* `A1` (bit 15) — ASID select. Fixed `0` in v0.1.
...
`MAIR_ACR1` provides 8 independent 8-bit attribute encodings (`Attr0..Attr7`), packed into a 64-bit register. In v0.1,
...
Other `AttrIdx` values are reserved in v0.1 and MUST fault if used in a valid translation entry.
...
For 4 KiB pages with 48-bit VA, the v0.1 bring-up profile uses the following index bits:
...
Descriptor type (v0.1):

* Invalid: `Desc[1:0] = 00`
* Table: `Desc[1:0] = 11` (points to next-level table)
* Block: `Desc[1:0] = 10` (maps a range at `L1` or `L2`; optional in v0.1)
...
Permission interpretation (v0.1):
...
NOTE: For v0.1, software SHOULD assume that `Block` descriptors are not implemented unless explicitly indicated by the
...
`TRAPARG0_ACRm`. The `TRAPNO_ACRm.CAUSE` encoding for these faults is profile-defined; in the v0.1 bring-up profile,
implementations SHOULD report `CAUSE=MMU_PF` for translation faults and `CAUSE=MMU_PERM` for permission faults.

[[system-iommu]]
==== IOMMU (v0.1 bring-up profile)
...
the v0.1 bring-up profile, implementations SHOULD report `CAUSE=IOMMU_PF`.
